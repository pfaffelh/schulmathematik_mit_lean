...
/- 
  Eine weitere Taktik werden wir noch oft brauchen: _rw_ (für rewrite); siehe Skript. Hier ein Beispiel:
-/
...
begin
  let f : X → Prop := λ x, true, 
  let g : X → Prop := λ x, false,
  use [f,g],
  intro x, 
  split, 
  intros h1 h2, use h2,  
...
begin
--  exact classical.skolem.mp,
  -- exact classical.axiom_of_choice, 
  -- oder
  intro h, 
  choose f hf using h, 
  use f, exact hf, 
--  use λ x, (classical.some (h x)), 
--  intro x, 
--  exact (classical.some_spec (h x)),
end
