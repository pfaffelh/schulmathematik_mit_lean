...
begin
  apply eq.symm,
  -- sorry,
  calc (n + 1)^2 - 1 = n^2 + 2*n + 1 - 1 : by {rw binom1,}
  ... = n^2 + 2*n : by {rw add_assoc, rw add_succ_sub_one, }
  ... = n*(n+2) : by {rw mul_comm 2 n, rw mul_add, rw pow_two,}
end
...
--  sorry,
  ...
  },
  {
    have h : 1 ≤ n+1, 
    { exact le_add_self, },
    calc (n+1) = 1 * (n+1) : by {rw one_mul,}
    ... ≤ (n+1)*(n+1) : by { exact mul_le_mul_right' h (n + 1), }
    ... = (n+1)^2 : by {rw pow_two,},
  },
end
...
begin
  ...
  change a + b - (b + c) = a - c,
  rw add_comm, 
  apply add_tsub_add_eq_tsub_left,  
end
...
begin
  ...
  have h2 : k^2 ≤ m^2, 
  {
    exact (pow_le_iff_le_left one_le_two).2 hkm,
  },
  have h3 : m^2 ≤ n^2, 
  {
    exact (pow_le_iff_le_left one_le_two).2 hmn,
  },
  have h4 : m^2 ≤ n^3, 
  {
